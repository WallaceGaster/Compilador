...
class CompilerIDE(QMainWindow):
    ...
    def run_lexical(self):
        ...
        if editor: # Si hay un editor abierto
            ...
            self.errores_lexicos = errors
            self.actualizar_estado_errores()
    ...
    def execute_p_code(self):
        ...
        # Verificar que el código P tenga contenido válido
        if not self.es_codigo_p_valido(codigo_p):
            QMessageBox.warning(
                self,
                "Código P inválido",
                "El código P generado no es válido. Regenere el código intermedio."
            )
            return
    ...
    def es_codigo_p_valido(self, codigo_p):
        """Verifica que el código P tenga instrucciones válidas"""
        lineas_validas = 0
        for linea in codigo_p.strip().split('\n'):
            linea_limpia = linea.strip()
            # Contar solo líneas que no sean comentarios y tengan contenido
            if linea_limpia and not linea_limpia.startswith(';'):
                if ':' in linea_limpia:  # Línea con número
                    instruccion = linea_limpia.split(':', 1)[1].strip()
                else:
                    instruccion = linea_limpia
                
                if instruccion and not instruccion.startswith(';'):
                    lineas_validas += 1
        
        # Requerir al menos algunas instrucciones básicas
        return lineas_validas >= 3  # Mínimo para un programa básico

    def mostrar_mensaje_errores(self):
        """Muestra un mensaje detallado sobre los errores encontrados"""
        mensaje = "No se puede generar/ejecutar código P debido a errores:\n\n"
        
        if self.errores_lexicos:
            mensaje += f"• Errores Léxicos: {len(self.errores_lexicos)} errores\n"
        
        if self.errores_sintacticos:
            mensaje += f"• Errores Sintácticos: {len(self.errores_sintacticos)} errores\n"
        
        if self.errores_semanticos:
            mensaje += f"• Errores Semánticos: {len(self.errores_semanticos)} errores\n"
        
        mensaje += "\nPor favor, corrija todos los errores antes de continuar."
        
        QMessageBox.warning(self, "Errores detectados", mensaje)
        
        # Resaltar la pestaña de errores
        for i in range(self.tabs.count()):
            if self.tabs.tabText(i) == "Errores":
                self.tabs.setCurrentIndex(i)
                break
    ...
    def run_syntax(self):
        ...
        self.errores_sintacticos = errors
        self.actualizar_estado_errores()
    ...
    def actualizar_estado_errores(self):
        """Actualiza el estado de errores global"""
        total_errores = len(self.errores_lexicos) + len(self.errores_sintacticos) + len(self.errores_semanticos)
        self.tiene_errores = total_errores > 0
        
        # Actualizar barra de estado con información de errores
        if self.tiene_errores:
            self.statusBar().showMessage(
                f"Errores: Léxicos({len(self.errores_lexicos)}) | "
                f"Sintácticos({len(self.errores_sintacticos)}) | "
                f"Semánticos({len(self.errores_semanticos)})"
            )
        else:
            self.statusBar().showMessage("Sin errores - Listo para generar código P")    
    ...
    def run_intermediate(self):
        ...
        if not hasattr(self, 'ast_anotado') or self.ast_anotado is None:
            QMessageBox.warning(
                self, 
                "Error", 
                "Debe ejecutar análisis semántico primero para generar el AST anotado."
            )
            return
        ...
        try:
            ...
            QMessageBox.information(
                self, 
                "Éxito", 
                "Código intermedio ejecutable generado exitosamente en programa.P\n\n"
                "Puede ejecutar el código P desde el menú Ejecutar."
            )
            self.statusBar().showMessage("Código P generado exitosamente")

        except Exception as e:
            QMessageBox.critical(
                self, 
                "Error en generación de código", 
                f"No se pudo generar el código P:\n{str(e)}"
            )
            import traceback
    ...
    def run_execution(self):
        ...
        # Limpiar listas de errores previos
        self.errores_lexicos = []
        self.errores_sintacticos = []
        self.errores_semanticos = []
        self.tiene_errores = False
        ...
        if self.errores_lexicos:
            self.error_list.addItem("Se encontraron errores léxicos. Revise y corrija.")
            self.tabs.setCurrentIndex(self.tabs.indexOf(self.error_list))
            return
        ...
        if self.errores_sintacticos:
            self.error_list.addItem("Se encontraron errores sintácticos. Revise y corrija.")
            self.tabs.setCurrentIndex(self.tabs.indexOf(self.error_list))
            return
        ...
        if self.errores_semanticos:
            self.error_list.addItem("Se encontraron errores semánticos. Revise y corrija.")
            self.tabs.setCurrentIndex(self.tabs.indexOf(self.error_list))
            return
        ...
        QMessageBox.information(
            self, 
            "Análisis completado", 
            "Todos los análisis se completaron sin errores.\n"
            "Ahora puede generar código intermedio P."
        )
